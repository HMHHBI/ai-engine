...
class ChatRepository:
    ...
    def add_message(
        ...
        cloud_urls = []
        if image_data_list:
            # Parse stringified json if passed as string
            images = (
                json.loads(image_data_list)
                if isinstance(image_data_list, str)
                else image_data_list
            )
            # FIX: Loop on parsed 'images' variable instead of 'image_data_list'
            for img_base64 in images:
                if str(img_base64).startswith("http"):
        ...
        new_msg = Message(
            chat_id=chat_id, role=role, content=content, image_data=db_image_data
        )
        db.add(new_msg)
        db.commit()
        return new_msg
    ...
    @staticmethod
    def save_ai_log(
        db: Session, task: str, prompt: str, response: str, user_id: int = None
    ):
    ...
    def delete_messages_after(
        db: Session, chat_id: int, user_id: int, after_index: int
    ):
        messages = (
            db.query(Message)
            .filter(Message.chat_id == chat_id)
        ...
        if after_index < len(messages):
            ...
            db.query(Message).filter(
                Message.chat_id == chat_id, Message.id >= target_msg_id

...
def test_build_result_matches_p2_08_schema():
    ...
    report = build_benchmark_report(
        ...
        resource_analysis_dict={
            ...
            "rss": {"peak_mib": 250.0, "aggregate_delta_mib": 2.0, "status": "bounded"},
            "vms": {"peak_mib": 500.0, "aggregate_delta_mib": 4.0, "status": "bounded"},
            "file_descriptors": {
                "peak": 30,
                "aggregate_delta": 0,
                "status": "cleanly-recycled",
            },
            "threads": {"peak": 15, "aggregate_delta": 0, "status": "bounded"},
            "database_pool": {
                "exhaustion_timeouts": 0,
                "status": "not directly instrumented; 0 connection errors observed",
            },
            "redis": {
                "connection_drops": 0,
                "status": "not directly instrumented; 0 connection errors observed",
    ...
    assert "not directly instrumented" in res_analysis["database_pool"]["status"]
    assert "not directly instrumented" in res_analysis["redis"]["status"]

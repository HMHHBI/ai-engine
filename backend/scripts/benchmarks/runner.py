...
from .scenarios import benchmark_health_live, benchmark_chat_non_rag
...
def render_markdown(results: List[dict], timestamp: str) -> str:
    md = [
        ...
        "",
        "| Scenario | Concurrency | Iterations | Latency p50 (ms) | Latency p95 (ms) | TTFT p50 (ms) | Chunks/sec | Errors |",
        "| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |",
    ]
    for r in results:
        ...
        md.append(
            ...
            f"{m['p50']} | {m['p95']} | {ttft_p50} | {chunks_sec} | {r['errors']} |"
        )
    md.append("")
    return "\n".join(md)
...
async def run_scenario(
    scenario_name: str, config: BenchmarkConfig, warmup: int
) -> ScenarioResult:
    ...
    async with create_benchmark_client(config.base_url, config.transport) as client:
        ...
        if scenario_name != "health_live":
            ...
            # Create a dedicated chat session
            chat_resp = await client.post(
                "/chat/new",
                json={"title": "Benchmark Non-RAG Session"},
                headers=headers,
            )
            if chat_resp.status_code == 200:
                ...
                raise RuntimeError(
                    f"Failed to create benchmark chat session: {chat_resp.status_code} {chat_resp.text}"
                )

        # Warmup loop (metrics discarded)
        if warmup > 0:
            print(
                f"Running {warmup} warmup iterations (excluded from reported percentiles)..."
            )
            warmup_res = ScenarioResult(
                scenario="warmup",
                iterations=warmup,
                concurrency=1,
                transport=config.transport,
            ...
            for _ in range(warmup):
                if scenario_name == "health_live":
                    ...
                    await benchmark_chat_non_rag(
                        client, warmup_res, headers=headers, chat_id=chat_id
                    )

        print(
            f"Executing {config.iterations} measurement iterations (concurrency={config.concurrency})..."
        )
        ...
        async def worker():
            async with sem:
                if scenario_name == "health_live":
                    ...
                    await benchmark_chat_non_rag(
                        client, result, headers=headers, chat_id=chat_id
                    )
...
def main():
    ...
    parser.add_argument(
        "--iterations", type=int, default=10, help="Total measurement iterations"
    )
    parser.add_argument(
        "--warmup", type=int, default=2, help="Warmup iterations (excluded from stats)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--scenario",
        default="health_live",
        choices=["health_live", "chat_non_rag"],
        help="Scenario",
    )
    parser.add_argument(
    ...
    res = asyncio.run(run_scenario(args.scenario, config, args.warmup))
    summary = res.summary()

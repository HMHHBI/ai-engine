...
class ScenarioResult:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        total_stream_sec = sum(self.stream_duration_ms) / 1000.0 if self.stream_duration_ms else 0.0
        ...
        return {
            ...
            "metrics": {
                ...
                "retrieval_ms": {
                    "p50": round(calculate_percentile(self.retrieval_ms, 50), 2),
                    "p95": round(calculate_percentile(self.retrieval_ms, 95), 2),
                    "p99": round(calculate_percentile(self.retrieval_ms, 99), 2),
                } if self.retrieval_ms else None,
                "ttft_ms": {
                ...
                "stream_duration_ms": {
                    "p50": round(calculate_percentile(self.stream_duration_ms, 50), 2),
                    "p95": round(calculate_percentile(self.stream_duration_ms, 95), 2),
                    "p99": round(calculate_percentile(self.stream_duration_ms, 99), 2),
                } if has_streams else None,
                "streaming_rates": {
                    "chunks_per_sec": round(total_chunks / total_stream_sec, 2) if total_stream_sec > 0 else 0.0,
                    "characters_per_sec": round(total_chars / total_stream_sec, 2) if total_stream_sec > 0 else 0.0,
                    "avg_chunks_per_response": round(total_chunks / len(self.chunk_counts), 2) if self.chunk_counts else 0.0,
                } if has_streams else None,
                "system": {
                    "cpu_percent_avg": round(sum(self.cpu_percent) / len(self.cpu_percent), 2) if self.cpu_percent else 0.0,
